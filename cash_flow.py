...
def find_data_structure(df):
    ...
    # Search for cash flow data start
    for idx, row in df.iterrows():
...
def load_cash_flow_file(file, file_extension='.xls'):
...
def analyze_cash_flow(df):
    ...
    try:
        ...
        # Calculate ratios if both operating and investing activities are found
        if 'operating_activities' in found_metrics and 'investing_activities' in found_metrics:
            op_col = found_metrics['operating_activities']
            inv_col = found_metrics['investing_activities']
            
            analysis['Op. CF to Inv. CF Ratio'] = (
                df[op_col] / df[inv_col].replace(0, np.nan)
            )
...
def visualize_trends(df):
    ...
    try:
        ...
        ax.set_title('Cash Flow Trends Over Time', fontsize=16, fontweight='bold')
...
def process_cash_flow_file(file, file_extension='.xls'):
    """Enhanced processing function with better error handling."""
    try:
        print(f"Processing cash flow file with extension: {file_extension}")
        
        cash_flow_df = load_cash_flow_file(file, file_extension)
        if cash_flow_df is None:
            print("Failed to load cash flow data")
            return None, None, None, None
        
        print(f"Loaded data shape: {cash_flow_df.shape}")
        
        rearranged_df = rearrange_data(cash_flow_df)
        if rearranged_df is None:
            print("Failed to rearrange data")
            return cash_flow_df, None, None, None
        
        analysis_df = analyze_cash_flow(rearranged_df)
        fig = visualize_trends(rearranged_df)
        
        print("Cash flow processing completed successfully")
        return cash_flow_df, rearranged_df, analysis_df, fig
        
    except Exception as e:
        print(f"Error in process_cash_flow_file: {e}")
        import traceback
        traceback.print_exc()
        return None, None, None, None
